...
public class TicketMasterAPI {
	...
	// Additional details of attributes needed under discovery v2 events
	...
	private String getAddress(JSONObject event) throws JSONException {
		if (!event.isNull(EMBEDDED)) {
			// Get info under _embedded
			JSONObject embedded = event.getJSONObject(EMBEDDED);
			// Get address under _embedded
			if (!embedded.isNull(VENUES)) {
				JSONArray venues = embedded.getJSONArray(VENUES);
				// Iterate over detailed info under venues
	...
	// Get category info of event
	private Set<String> getCategories(JSONObject event) throws JSONException {
		Set<String> categories = new HashSet<>();
		if (!event.isNull(CLASSIFICATIONS)) {
			JSONArray classifications = event.getJSONArray(CLASSIFICATIONS);
			// An event may fit in multiple categories tags
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		String query = String.format("apikey=%s&geoPoint=%s&keyword=%s&radius=50", API_KEY, geoHash, keyword, 50);
		...
		try {
			...
			if (!obj.isNull(EMBEDDED)) {
				// Get all the events under _embedded and return a list of Item
				JSONObject embbeded = obj.getJSONObject(EMBEDDED);
	...
	private void queryAPI(double lat, double lon) {
		...
		// Print out all the events from the API for testing
		try {
	...
	public static void main(String[] args) {
		...
		// Events near Oracle Arena
		// api.queryAPI(37.7503, -122.2030);
	}
